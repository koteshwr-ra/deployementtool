...
final class SSLUtils {
    ...
    static final boolean USE_ENGINE_SOCKET_BY_DEFAULT = Boolean.parseBoolean(System.getProperty("org.conscrypt.useEngineSocketByDefault", Bugly.SDK_IS_DEV));
    ...
    static Set<String> getSupportedClientKeyTypes(byte[] bArr, int[] iArr) {
        ...
        for (byte clientKeyType : bArr) {
            String clientKeyType2 = getClientKeyType(clientKeyType);
            if (clientKeyType2 != null) {
                hashSet.add(clientKeyType2);
            }
        }
        LinkedHashSet linkedHashSet = new LinkedHashSet(iArr.length);
        for (int clientKeyTypeFromSignatureAlg : iArr) {
            String clientKeyTypeFromSignatureAlg2 = getClientKeyTypeFromSignatureAlg(clientKeyTypeFromSignatureAlg);
            if (clientKeyTypeFromSignatureAlg2 != null) {
                linkedHashSet.add(clientKeyTypeFromSignatureAlg2);
            }
        }
            ...
            while (i < strArr.length) {
                if (strArr[i] != null) {
                    ...
                    if (length == 0 || length > 255) {
                        throw new IllegalArgumentException("protocol[" + i + "] has invalid length: " + length);
                    }
                    i2 += length + 1;
                    i++;
                } else {
