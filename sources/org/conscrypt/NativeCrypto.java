...
public final class NativeCrypto {
    ...
    private static int X509_NAME_hash(X500Principal x500Principal, String str) {
        try {
            ...
            return ((digest[3] & 255) << Ascii.CAN) | ((digest[1] & 255) << 8) | ((digest[0] & 255) << 0) | ((digest[2] & 255) << Ascii.DLE);
        ...
        for (String str2 : strArr) {
            if (!str2.equals(TLS_EMPTY_RENEGOTIATION_INFO_SCSV)) {
                if (!str2.equals(TLS_FALLBACK_SCSV) || (!str.equals(SUPPORTED_PROTOCOL_TLSV1) && !str.equals(SUPPORTED_PROTOCOL_TLSV1_1))) {
                    ...
                    SSL_set_mode(j, nativeSsl, PlaybackStateCompat.ACTION_PLAY_FROM_MEDIA_ID);
                }
            }
        }
    ...
    static String[] checkEnabledCipherSuites(String[] strArr) {
        if (strArr != null) {
            ...
            while (i < strArr.length) {
                ...
                } else if (strArr[i].equals(TLS_EMPTY_RENEGOTIATION_INFO_SCSV) || strArr[i].equals(TLS_FALLBACK_SCSV) || SUPPORTED_TLS_1_2_CIPHER_SUITES_SET.contains(strArr[i]) || SUPPORTED_LEGACY_CIPHER_SUITES_SET.contains(strArr[i])) {
                    i++;
                } else {
